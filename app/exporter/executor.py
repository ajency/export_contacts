...
class Executor():
    def __init__(self, env, auto, headless, proxy_list, account):
        self.env = env
        self.auto = auto
    ...
    def step_linkedIn_login(self):
        emit("action", "Step: linkedIn_login")
        if self.linkedInHandle.login():
            emit('action', 'Login to Linked in Successfull!')
            return True
        else:
            self.screenshot.capture('linkedIn_login_error')
            emit('action', 'Error linkedIn login. Check screenshots for details.')
            return False

    def step_email_operation(self, sequences):
        emit("action", "Step: email_operation")
        total_email_count = 0
            ...
            for email_account in provider:
                ...
                for sequence in sequences:
                    ...
                    if is_success:
                        ...
                        emit('action','Error performing '+sequence+' for email id: '+email_account.get('username')+'. Skipping....')
                        break
        if total_email_count == failed_email_count:
            emit('action', 'Error email operation, all email failed.')
            return False
        else:
            emit('action', 'Email operation successfull!')
            return True

    def step_linkedIn_logout(self):
        emit("action", "Step: linkedIn_logout")
        if self.linkedInHandle.logout():
            emit('action', 'Linked in logout successfull!')
            return True
        else:
            self.screenshot.capture('linkedIn_logout_error')
            emit('action', 'Error logging out from linkedIn. Check screenshots for details.')
            return False



    def step_email_login(self, provider, email, last_entry):
        emit("action", "Step: email_login")
        emit('action','Performing email login with id '+email.get('username'))
        return getattr(self, 'email_login_' + provider)(email)

    def step_import_contacts(self, provider, email, last_entry):
        emit("action", "Step: import_contacts")
        if self.linkedInHandle.import_contacts(provider, email):
            emit('action', 'Import contacts successful for '+provider+' with email: '+email.get('username'))
            return True
        else:
            self.screenshot.capture('import_contacts_error_'+email.get('username'))
            emit('action', 'Import contacts failed for '+provider+' with email: '+email.get('username'))
            return False

    def step_export_contacts(self, provider, email, last_entry):
        emit("action", "Step: export_contacts")
        if self.linkedInHandle.export_contacts(email):
            emit('action', 'Export contacts successful for ' + provider + ' with email: ' + email.get('username'))
            return True
        else:
            self.screenshot.capture('export_contacts_error_' + email.get('username'))
            emit('action', 'Export contacts failed for ' + provider + ' with email: ' + email.get('username'))
            return False

    def step_delete_contacts(self, provider, email, last_entry):
        emit("action", "Step: delete_contacts")
        if self.linkedInHandle.delete_contacts(last_entry):
            emit('action', 'Delete contacts successful for ' + provider + ' with email: ' + email.get('username'))
            time.sleep(10)
            return True
        else:
            self.screenshot.capture('delete_contacts_error_' + email.get('username'))
            emit('action', 'Delete contacts failed for ' + provider + ' with email: ' + email.get('username'))
            return False

    def step_email_logout(self, provider, email, last_entry):
        emit("action", "Step: email_logout")
    ...
    def email_login_gmail(self,email):
        ...
        if self.gmailHandle.login(email):
            emit('action', 'Login to gmail successful!')
            return True
        else:
            self.screenshot.capture('gmail_login_error_' + email.get('username'))
            emit('action', 'Error gmail login. Check screenshots for details.')
            return False
    ...
    def email_login_yahoo(self,email):
        ...
        if self.yahooHandle.login(email, 'yahoo'):
            emit('action', 'Login to yahoo successful!')
            return True
        else:
            self.screenshot.capture('yahoo_login_error_'+email.get('username'))
            emit('action', 'Error yahoo login. Check screenshots for details.')
            return False
    ...
    def email_login_aol(self,email):
        ...
        if self.aolHandle.login(email, 'aol'):
            emit('action', 'Login to aol successful!')
            return True
        else:
            self.screenshot.capture('aol_login_error_' + email.get('username'))
            emit('action', 'Error aol login. Check screenshots for details.')
            return False
