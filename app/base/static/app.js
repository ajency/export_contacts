...
$("input[name='environment']").on("change", function(event){
 ...
 $("#initiate_form_btn").on("click", function(event){
  event.preventDefault();
    ...
    if( $("input:checked[name='environment']").length ){
        var env = $("input:checked[name='environment']").val();
    ...
    if( $("input:checked[name='auto']").length ){
        var auto = $("input:checked[name='auto']").val();
        payload['auto'] = (auto == '1')? true : false;
    }

    if( $("input:checked[name='headless_mode']").length ){
        var headless_mode = $("input:checked[name='headless_mode']").val();
        payload['headless'] = (headless_mode == '1')? true : false;
    }

    if( $("#accounts_config").length ){
    ...
    if(payload.auto){
        ...
        if(payload.hasOwnProperty('batch_id')){
            socket.emit('start_batch', payload.batch_id);
        }else{
